...
import argparse
from exp.exp2.dataset import myImageFolder
from tqdm import tqdm
from exp.exp1.models import *
from utils import *
...
if __name__ == "__main__":
    ...
    trainset = myImageFolder(root='G:\\DIV2K-train', HR='F:\\DIV2K\\DIV2K_train_HR', transform=transform_train)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=16, shuffle=True, num_workers=4)

    testset = myImageFolder(root='G:\\DIV2K-test', HR='F:\\DIV2K\\DIV2K_train_HR', transform=transform_test)
    testloader = torch.utils.data.DataLoader(testset, batch_size=16, shuffle=False, num_workers=4)
    classes = ('blur', 'noise')

    # Model
    print('==> Building models..')
    net = get_network(args, input_channels=6)
    net = net.to(device)
    if device == 'cuda':
    ...
    scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, milestones=[20, 40, 60, 80, 100],
                                                     gamma=0.1)
    ...
    def train(epoch):
        ...
        for batch_idx, (LR, HR, targets) in progressbar:
            LR, HR, targets, = LR.to(device), HR.to(device), targets.to(device)
            optimizer.zero_grad()
            outputs = net(torch.cat([LR, HR], dim=1))
